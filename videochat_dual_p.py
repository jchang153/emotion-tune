...
from gui_p import ChatApp, VideoPlayerWorker, create_emotion_survey, create_chat_evaluation
from emili_core_dual_p import * # core threading logic
...
if __name__ == "__main__":
    ...
    EMA_thread = threading.Thread(
        target=EMA_thread, 
        args=(start_time, snapshot_path, pipeline, transcript_path, start_time_str, model_name, gui_app), 
        daemon=True)
    EMA_thread.start()

    use_anthropic = True
    print(f"Anthropic value: {use_anthropic}")
    
    sender_thread = threading.Thread(
        target=sender_thread,
        args=(model_name, max_context_length, gui_app, transcript_path,
